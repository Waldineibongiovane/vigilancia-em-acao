...
import { LayoutDashboard, LogOut, PanelLeft, AlertTriangle, TrendingUp, FileText, Settings, Home, Shield } from "lucide-react";
...
const menuItems = [
  ...
  { icon: AlertTriangle, label: "Denúncias", path: "/admin?tab=complaints" },
  { icon: TrendingUp, label: "Métricas", path: "/admin?tab=metrics" },
  { icon: FileText, label: "Boletins", path: "/admin?tab=bulletins" },
  { icon: Settings, label: "Configurações", path: "/admin?tab=settings" },
];
        ...
        <Sidebar collapsible="icon" className="border-r-0" disableTransition={isResizing}>
            ...
            <div className="flex items-center gap-3 px-2 transition-all w-full">
              ...
              {!isCollapsed && (
                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-semibold tracking-tight truncate text-primary">Admin</span>
                </div>
              )}
            </div>
          </SidebarHeader>
          ...
          <SidebarContent className="gap-0">
            <SidebarMenu className="px-2 py-1">
              ...
              {/* Back to public site */}
              <SidebarMenuItem>
                <SidebarMenuButton onClick={() => setLocation("/")} tooltip="Voltar ao site" className="h-10 transition-all font-normal">
                  <Home className="h-4 w-4" />
        ...
        {isMobile && (
          <div className="flex border-b h-14 items-center justify-between bg-background/95 px-2 backdrop-blur supports-[backdrop-filter]:backdrop-blur sticky top-0 z-40">
            <div className="flex items-center gap-2">
              ...
              <span className="tracking-tight text-foreground">{activeMenuItem?.label ?? "Admin"}</span>
            </div>
          </div>
        )}
